...
function readDefinitions(root, callback) {
    const read = (type, cb) => async.apply(readFiles, path.resolve(root, type), cb);
    async.parallel({
        domains: read('domains', Domain.normalize),
        nodes: read('nodes', Node.normalize),
        services: read('services', s => Service.normalize(s, root)),
        solutions: read('solutions', Solution.normalize),
    }, callback);
}
 ...
 * @param {function(object)} normalizeCallback - Function that normalizes read config.
 * @param {function(?object, ?object=)} callback - Called when done.
 */
function readFiles(root, normalizeCallback, callback) {
...
/**
 * Calculate a 'score' for a setting vale.
 *
 * @param {GuardedSettingValue} value - The setting being evaluated
 ...
 */
function calculateSettingValueScore(value, domain, node) {
...
/**
 * Select the best matching setting value
 * It calculates scores for all the possible setting values
 * Highest score wins and becomes the value of the setting.
 *
 ...
 */
function selectSettingValue(setting, domain, node, service, errors) {
    let candidates = service.settings[setting].map(v => {
        return {
            value: v.value,
            score: calculateSettingValueScore(v, domain, node)
        };
    ...
    candidates.filter(v => v.score === best.score && v.value !== best.value).forEach(v => {
        errors.push({
            level: 'warn',
            message: `domain '${domain.name}', service '${service.name}': setting '${setting}': ` +
                `choosing '${best.value}' over '${v.value}' even though ` +
...
/**
 * Resolve service settings values.
 *
 * @param {Domain} domain - Current domain
 * @param {NodeConfig} node - Current node
 * @param {Service} service - The service object the settings are for
 * @param {Array.<DeliveryTreeError>} errors - Place to collect errors / warnings
 * @return {object} - Prepared delivery tree part for the given domain
 */
function selectServiceSettings(domain, node, service, errors) {
    const newSettings = {};
    Object.keys(service.settings).forEach(s => {
        const value = selectSettingValue(s, domain, node, service, errors);
        if(value !== undefined) {
            newSettings[s] = value;
        }
    });
...
function createServiceDeliveryTree(domain, node, service, errors) {
    return {
        ...service,
        settings: selectServiceSettings(domain, node, service, errors)
    };
}
...
module.exports.calculateSettingValueScore = calculateSettingValueScore;
module.exports.selectSettingValue = selectSettingValue;
module.exports.selectServiceSettings = selectServiceSettings;
