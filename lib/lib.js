...
function helpText(message) {
    ...
    help +=
        ...
        'resources [[-d|--delete ${ResourceName}]|[--upload ${FilePath}]] [[-x|--xslt]|[-j|--java]] [settings]\n' +
        'variables [settings]\n' +
        ...
        'customnotes ${ServiceName} [[${OutputFile}]|[--upload ${InputFile}]] [settings]\n' +
        'repository ${ServiceName} [${Destination}] [settings]\n' +
        '--help\n\n' +
        'settings:\n' +
...
function getVariant(options) {
    return options.xslt ? XSLT_RESOURCE_TYPE : options.java ? JAVA_RESOURCE_TYPE : GENERAL_RESOURCE_TYPE;
}
...
module.exports.perform = function(options, callback){
    ...
    switch(options.operation){
        ...
        case operations.CUSTOMNOTES:
            if(options.nodejs || options.java){
                return callback({ errorType: 'Logic error', error: 'Custom model notes are only supported for xUML services'});
            }
            if(options.upload) {
        ...
        case operations.RESOURCES:
            if(options['delete']) {
                bridgeInstance.deleteXUMLResources(getVariant(options), options.resource, callback);
            } else if(options.upload) {
                bridgeInstance.uploadXUMLResources(getVariant(options), options.resource, callback);
            } else {
                bridgeInstance.listXUMLResources(getVariant(options), callback);
            }
            return null;
...
module.exports.isXslt = function(argv) {
    return (argv['x'] || argv['xslt']) && true;
};
